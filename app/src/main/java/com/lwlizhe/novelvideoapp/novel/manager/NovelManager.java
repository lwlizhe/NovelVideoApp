package com.lwlizhe.novelvideoapp.novel.manager;
...
public class NovelManager {
    ...
    private static volatile NovelManager sInstance;

    public static NovelManager getInstance(){
        if (sInstance == null){
            synchronized (NovelManager.class){
                if (sInstance == null){
                    sInstance = new NovelManager();
                }
            }
        }
        return sInstance;
    ...
    public boolean openChapter(String bookId,String chapterName,long position){
        //如果文件不存在，则打开失败
        File file = new File(GlobeConstance.BOOK_CACHE_PATH + bookId
                + File.separator + chapterName + FileUtils.SUFFIX_CUSTOM);
        if (!file.exists()){
            return false;
        }
        this.bookId = bookId;
    ...
    public static File getBookFile(String folderName, String fileName){
        return FileUtils.getFile(GlobeConstance.BOOK_CACHE_PATH + folderName
                + File.separator + fileName + FileUtils.SUFFIX_CUSTOM);
    }

//    public static long getBookSize(String folderName){
//        return FileUtils.getDirSize(FileUtils
//                .getFolder(GlobeConstance.BOOK_CACHE_PATH + folderName));
//    }
    ...
    public static boolean isChapterCached(String folderName, String fileName){
        File file = new File(GlobeConstance.BOOK_CACHE_PATH + folderName
                + File.separator + fileName + FileUtils.SUFFIX_CUSTOM);
        return file.exists();
    }
