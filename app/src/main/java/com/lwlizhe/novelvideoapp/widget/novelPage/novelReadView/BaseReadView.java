...
public abstract class BaseReadView extends View {
    ...
    public BaseReadView(Context context, String bookId, List<NovelChapterEntity> chaptersList,
    ...
    public synchronized void init(int theme,int chosedChpater,int chosedVolume) {
        if (!isPrepared) {
            try {
//                pagefactory.setBgBitmap(ThemeManager.getThemeDrawable(theme));

                if(chosedChpater==-1||chosedVolume==-1){
                    // 自动跳转到上次阅读位置
                    int pos[] = new int[]{0};
//                    int pos[] = SettingManager.getInstance().getReadProgress(bookId);
                    int ret = pagefactory.openBook(pos[0],pos[1] ,new int[]{pos[2], pos[3]});
                    LogUtils.debugInfo("上次阅读位置：chapter=" + pos[0] +" volume="+pos[1] +" startPos=" + pos[2] + " endPos=" + pos[3]);
                    if (ret == 0) {
                        listener.onLoadChapterFailure(pos[0],pos[1]);
                        return;
                    }
                    pagefactory.onDraw(mCurrentPageCanvas);
                    postInvalidate();
                }else{
                    int ret = pagefactory.openBook(chosedChpater,chosedVolume ,new int[]{0,0});
                    if (ret == 0) {
                        listener.onLoadChapterFailure(chosedChpater,chosedVolume);
                        return;
                    }
                    pagefactory.onDraw(mCurrentPageCanvas);
                    postInvalidate();
                ...
                LogUtils.debugInfo("Error",e.getMessage());
            }
            isPrepared = true;
        }
    }
...
//                            ToastUtils.showSingleToast("没有下一页啦");
    ...
    public void jumpToChapter(int chapter,int volume) {
        resetTouchPoint();
        pagefactory.openBook(chapter,volume, new int[]{0, 0});
        pagefactory.onDraw(mCurrentPageCanvas);
        pagefactory.onDraw(mNextPageCanvas);
        postInvalidate();
    }
