...
import com.lwlizhe.basemodule.utils.LogUtils;
...
public class PageFactory {
    ...
    private int currentChapterPos, tempChapterPos;
    private int currentVolumePos, tempVolumePos;
    ...
    private List<NovelChapterEntity> chaptersList;
    ...
    public PageFactory(Context context, String bookId, List<NovelChapterEntity> novels) {
        this(context, UiUtils.getScreenWidth(), UiUtils.getScreenHeight(),
    ...
    public PageFactory(Context context, int width, int height, int fontSize, String bookId,
                       List<NovelChapterEntity> novels) {
        mContext = context;
        mWidth = width;
        mHeight = height;
        mFontSize = fontSize;
        mLineSpace = mFontSize / 5 * 2;
        mNumFontSize = UiUtils.dpToPxInt(16);
        marginWidth = UiUtils.dpToPxInt(15);
        marginHeight = UiUtils.dpToPxInt(15);
        ...
        this.bookId = bookId;
        this.chaptersList = novels;

        time = dateFormat.format(new Date());
    }

    public File getBookFile(int chapter, int volume) {
        File file = FileUtils.getChapterFile(bookId, chapter, volume);
        if (file != null && file.length() > 10) {
            // 解决空文件造成编码错误的问题
            charset = FileUtils.getCharset(file.getAbsolutePath());
        }
        LogUtils.debugInfo("charset=" + charset);
        return file;
    }
//
...
//    }
     ...
     */
    public int openBook(int chapter, int volume, int[] position) {
        this.currentChapterPos = chapter;
        this.currentVolumePos = volume;
//        this.chapterSize = chaptersList.size();
//        if (currentChapterPos > chapterSize)
//            currentChapterPos = chapterSize;
        String path = getBookFile(currentChapterPos, volume).getPath();
        try {
            ...
            if (length > 10) {
                ...
                onChapterChanged(chapter,volume);
                mLines.clear();
                return 1;
            }
        } catch (IOException e) {
    ...
    public synchronized void onDraw(Canvas canvas) {
        ...
        if (mLines.size() > 0) {
            ...
            // 绘制标题
            canvas.drawText(chaptersList.get(currentVolumePos - 1).getChapters().get(currentChapterPos - 1).getChapter_name(), marginWidth, y, mTitlePaint);
            ...
            if (batteryBitmap != null) {
                canvas.drawBitmap(batteryBitmap, marginWidth + 2,
                        mHeight - marginHeight - UiUtils.dpToPxInt(12), mTitlePaint);
            }

//            float percent = (float) currentChapterPos * 100 / chapterSize;
...
//                    mHeight - marginHeight, mTitlePaint);
            ...
            // 保存阅读进度
//            SettingManager.getInstance().saveReadProgress(bookId, currentChapterPos, currentVolumePos, curBeginPos, curEndPos);
        }
    }
    ...
    public Vector<String> pageLast() {
        ...
        //SettingManager.getInstance().saveReadProgress(bookId, currentChapterPos, curBeginPos, curEndPos);
        return lines;
    }
    ...
    public boolean hasNextPage() {
        return currentVolumePos < chaptersList.size() || currentChapterPos < chaptersList.get(currentVolumePos-1).getChapters().size() || curEndPos < mbBufferLen;
    }

    public boolean hasPrePage() {
        return currentVolumePos > 1 || (currentVolumePos==1&&currentChapterPos>1)|| (currentVolumePos==1&&currentChapterPos == 1 && curBeginPos > 0);
    }
    ...
    public BookStatus nextPage() {
        if (!hasNextPage()) { // 最后一章的结束页
            ...
            tempChapterPos = currentChapterPos;
            tempVolumePos=currentVolumePos;
            tempBeginPos = curBeginPos;
            tempEndPos = curEndPos;
            if (curEndPos >= mbBufferLen) { // 中间章节结束页
                if (currentChapterPos < chaptersList.get(currentVolumePos-1).getChapters().size()) {
                    currentChapterPos++;
                } else {
                    currentVolumePos++;
                    currentChapterPos = 1;
                ...
                int ret = openBook(currentChapterPos, currentVolumePos, new int[]{0, 0}); // 打开下一章
                if (ret == 0) {
                    onLoadChapterFailure(currentChapterPos,currentVolumePos);
                    currentChapterPos=tempChapterPos;
                    currentVolumePos=tempVolumePos;
                    curBeginPos = tempBeginPos;
                    curEndPos = tempEndPos;
                    return BookStatus.NEXT_CHAPTER_LOAD_FAILURE;
                } else {
                    currentPage = 0;
                    if(currentVolumePos>tempVolumePos){
                        onVolumeChanged(currentVolumePos);
                    }
                    onChapterChanged(currentChapterPos,currentVolumePos);
                }
            } else {
            ...
            onPageChanged(currentChapterPos,currentVolumePos, ++currentPage);
        }
        ...
        if (!hasPrePage()) { // 第一章第一页
            ...
            if (curBeginPos <= 0) {
                if(currentChapterPos<=1){
                    currentVolumePos--;
                    currentChapterPos=chaptersList.get(currentVolumePos-1).getChapters().size();
                }else{
                    currentChapterPos--;
                }


                int ret = openBook(currentChapterPos, currentVolumePos, new int[]{0, 0});
                if (ret == 0) {
                    onLoadChapterFailure(currentChapterPos,currentVolumePos);
                    currentChapterPos=tempChapterPos;
                    currentVolumePos=tempVolumePos;
    ...
    public void cancelPage() {
        ...
        int ret = openBook(currentChapterPos, currentVolumePos, new int[]{curBeginPos, curEndPos});
        if (ret == 0) {
            onLoadChapterFailure(currentChapterPos,currentVolumePos);
            return;
        }
    ...
    public int[] getPosition() {
        return new int[]{currentChapterPos, curBeginPos, curEndPos};
    }
    ...
    public void setTextFont(int fontsize) {
        LogUtils.debugInfo("fontSize=" + fontsize);
        mFontSize = fontsize;
        mLineSpace = mFontSize / 5 * 2;
        mPaint.setTextSize(mFontSize);
        mPageLineCount = mVisibleHeight / (mFontSize + mLineSpace);
    ...
    private void onChapterChanged(int chapter,int volume) {
        if (listener != null)
            listener.onChapterChanged(chapter,volume);
    }
    ...
    private void onPageChanged(int chapter,int volume ,int page) {
        if (listener != null)
            listener.onPageChanged(chapter,volume ,page);
    }

    private void onLoadChapterFailure(int chapter,int volume) {
        if (listener != null)
            listener.onLoadChapterFailure(chapter,volume);
    }
    ...
    public void convertBetteryBitmap() {
        ...
        batteryView.measure(View.MeasureSpec.makeMeasureSpec(UiUtils.dpToPxInt(26), View.MeasureSpec.EXACTLY),
                View.MeasureSpec.makeMeasureSpec(UiUtils.dpToPxInt(14), View.MeasureSpec.EXACTLY));
    ...
    public void recycle() {
        if (mBookPageBg != null && !mBookPageBg.isRecycled()) {
            mBookPageBg.recycle();
            mBookPageBg = null;
            LogUtils.debugInfo("mBookPageBg recycle");
        }

        if (batteryBitmap != null && !batteryBitmap.isRecycled()) {
            batteryBitmap.recycle();
            batteryBitmap = null;
            LogUtils.debugInfo("batteryBitmap recycle");
        }
    }
}
