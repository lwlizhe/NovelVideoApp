...
public interface ComicNetService {
    ...
    // 推荐位
    @GET(GlobeConstance.CONSTANCE_URL.COMIC_BASE_URL+"v3/recommend.json")
    Flowable<List<ComicRecommendResponse>> getRecommend();

    // 漫画详情页
    @GET(GlobeConstance.CONSTANCE_URL.COMIC_BASE_URL+"comic/{id}.json")
    Flowable<Object> getComicInstruction(@Path("id") long novelId);

    // 漫画内容
    @GET(GlobeConstance.CONSTANCE_URL.COMIC_BASE_URL+"chapter/{comicId}/{chapterId}.json")
    ...
    // 漫画相关
    @GET(GlobeConstance.CONSTANCE_URL.COMIC_BASE_URL+"v3/comic/related/{id}.json")
    Flowable<Object> getComicRelated(@Path("id") long comicId);

    // 漫画评论
    @GET(GlobeConstance.CONSTANCE_URL.COMIC_BASE_URL+"comment2/4/0/{comicId}/1/{page}.json")
